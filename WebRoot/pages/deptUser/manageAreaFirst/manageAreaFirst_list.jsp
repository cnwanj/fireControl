...
<script type="text/javascript">
	...
	$(".sure").click(function(){
		$(".tip").fadeOut(100);
		$.post("<%= basePath%>/sys/manageAreaFirst_delete.action", {afId:id}, function(data){
			var url = "<%=basePath%>/sys/ManageAreaFirst_findByPage.action?userId=${userCode}";
			if(data.targ == true){
...
<div class="formbody">
    <div id="usual1" class="usual">
      <div id="tab2" class="tabson">
     	...
     	<form action="<%= basePath%>/sys/ManageAreaFirst_findByPage.action?userId=${userCode}" method="post" target="rightFrame">
    	<ul class="seachform">
    	    <li><label>搜索</label><input class="scinput" name="afName" placeholder="请输入名称"/></li>
            <li><input name="" type="submit" class="scbtn" value="搜索"/></li>
            	<li class="clickk"><a href="<%= basePath%>/sys/ManageAreaFirst_toManageAreaFirstAddPage.action?userId=${userCode}"><span><img src="<%=path%>/images/t01.png" /></span> 添加</a></li>
            <li>
    ...
    <table class="tablelist">
        ...
        <tbody>
	        ...
	        <tr>
		        ...
		        <td>${ufId}</td>
		        <td>&nbsp;
	        		<a href="javascript:;" class="tablelinkdelete" idValue="${afId}">删除</a>&nbsp;&nbsp;
	        		<a href="<%=basePath%>/sys/ManageAreaFirst_toManageAreaFirstUpdatePage.action?afId=${afId}&&userId=${userCode}" class="tablelink">修改</a>
		        </td>
	        </tr> 
	        </s:iterator>
        </tbody>
...
<%
//查询的url地址，统一写
String listActionURL = basePath+"/sys/ManageAreaFirst_findByPage.action";
%>
...
<script type="text/javascript">
var url = "<%=basePath%>/sys/ManageAreaFirst_findByPage.action";                 //获取表单url
//首页
function first(){

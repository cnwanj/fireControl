...
<script type="text/javascript">
	...
	$(".sure").click(function(){
		$(".tip").fadeOut(100);
		$.post("<%= basePath%>/sys/manageUserFirst_delete.action", {ufId:id}, function(data){
			var url = "<%=basePath%>/sys/ManageUserFirst_findByPage.action?deptUser.userId=${userCode}";
			if(data.targ == true){
...
<div class="formbody">
    <div id="usual1" class="usual">
      <div id="tab2" class="tabson">
     	...
     	<form action="<%= basePath%>/sys/ManageUserFirst_findByPage.action?deptUser.userId=${userCode}" method="post" target="rightFrame">
    	<ul class="seachform">
            ...
            <li><input name="" type="submit" class="scbtn" value="搜索"/></li>
            	<li class="clickk"><a href="<%= basePath%>/sys/ManageUserFirst_toManageUserFirstAddPage.action?userId=${userCode}"><span><img src="<%=path%>/images/t01.png" /></span> 添加</a></li>
            <li>
    ...
    <table class="tablelist">
        ...
        <tbody>
	        ...
	        <tr>
		        ...
		        <s:elseif test="ufType == 1">
		        	<td>管辖员</td>
		        </s:elseif>
		        <td>${deptUser.userId}</td>
		        <td>${af.afName}</td>
		        <td>&nbsp;
	        		<a href="javascript:;" class="tablelinkdelete" idValue="${ufId}">删除</a>&nbsp;&nbsp;
	        		<a href="<%=basePath%>/sys/ManageUserFirst_toManageUserFirstUpdatePage.action?ufId=${ufId}&&deptUserId=${userCode}" class="tablelink">修改</a>
		        </td>
	        </tr> 
	        </s:iterator>
        </tbody>
...
<%
//查询的url地址，统一写
String listActionURL = basePath+"/sys/ManageUserFirst_findByPage.action";
%>
...
<script type="text/javascript">
var url = "<%=basePath%>/sys/ManageUserFirst_findByPage.action";                 //获取表单url
//首页
function first(){

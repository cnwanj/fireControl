...
<script type="text/javascript">
	...
	$(".sure").click(function(){
		$(".tip").fadeOut(100);
		$.post("<%= basePath%>/sys/installPoint_delete.action", {insId:id}, function(data){
			var url = "<%=basePath%>/sys/InstallPoint_findByPage.action";
			if(data.targ == true){
...
<div class="formbody">
    <div id="usual1" class="usual">
      <div id="tab2" class="tabson">
     	...
     	<form action="<%= basePath%>/sys/InstallPoint_findByPage.action" method="post" target="rightFrame">
    	<ul class="seachform">
    	    <li><label>搜索</label><input class="scinput" name="insName" placeholder="请输入名称"/></li>
            <li><input name="" type="submit" class="scbtn" value="搜索"/></li>
            	<li class="clickk"><a href="<%= basePath%>/sys/InstallPoint_openSave.action"><span><img src="<%=path%>/images/t01.png" /></span> 添加</a></li>
            <li>
    ...
    <table class="tablelist">
        ...
        <tbody>
	        ...
	        <tr>
		        ...
		        <td>${as.asName}</td>
		        <td>&nbsp;
		        	<a href="javascript:;" class="tablelinkdelete" idValue="${insId}">删除</a>&nbsp;&nbsp;
	        		<!-- <a href="<%=basePath%>/sys/InstallPoint_delete.action?insId=${insId}">删除</a>&nbsp;&nbsp; -->
	        		<a href="<%=basePath%>/sys/InstallPoint_openUpdate.action?insId=${insId}" class="tablelink">修改</a>
		        </td>
	        </tr> 
	        </s:iterator>
        </tbody>
...
<%
//查询的url地址，统一写
String listActionURL = basePath+"/sys/InstallPoint_findByPage.action";
%>
...
<script type="text/javascript">
var url = "<%=basePath%>/sys/InstallPoint_findByPage.action";                 //获取表单url
//首页
function first(){
